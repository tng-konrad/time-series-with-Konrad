...
Both metrics are in units sold per day: MAE is the typical daily miss, RMSE the version that punishes big misses harder. Copy-yesterday versus copy-last-week, and last week wins by nearly a factor of two — the weekly cycle carries most of the predictable signal. **Seasonal naive is the bar: it costs nothing, trains nothing, and knows nothing except "weeks repeat."**

> **[FIGURE 3 — graphs/graph14-03.png — notebook cell 28: seasonal naive forecast vs actuals, first origin]**

It nails the *shape* — weekend peaks land on weekends — but copies last week's level and noise verbatim, wiggles and all. Those two failure modes, level drift and noise-copying, are the headroom any learned model has to work with.
...
```
linear:  1,190 parameters,  4.7s to train
GRU:    13,774 parameters, 23.7s to train
```
...
**Patching** — episode 13's big idea, industrialized. One `reshape` cuts a 512-day context into 16 non-overlapping blocks of 32 days; inside the real model each block is pushed through a small MLP (a little feed-forward network) to become one embedding vector, and *those* are the tokens.
...
One new tool before the ladder. Foundation models advertise *probabilistic* forecasts — we ask each for its 10th, 50th, and 90th percentiles, the values the model believes the actual will fall below 10%, 50%, and 90% of the time. That gives a median point forecast plus an **80% prediction interval** (the band between the 10th and 90th). Two questions need scoring. Is the interval *honest*? Coverage: the fraction of actuals that land inside the band, which should be about 80%. And is each quantile *sharp*? The **pinball loss** for quantile level τ:
...
The first rung. **TimesFM** is Google Research's flagship: a decoder-only transformer over patch tokens — "decoder-only" meaning it reads left to right and predicts what comes next, GPT-style, where episode 13's encoder blocks processed a whole window at once. It was pre-trained on roughly 100 billion real-world time points, much of it Google Trends and Wikipedia pageview series, chosen because web attention encodes the human weekly and holiday rhythms that transfer to domains like retail. Version 2.5, the current one, is leaner and stronger than its predecessors: 200M parameters (down from 500M), a context window stretched to 16,384 points, and no more manual frequency flag.
...
```
All 120 two-week forecasts in 6.5s — zero training

TimesFM 2.5: {'MAE': 4.95, 'RMSE': 5.55}
80% interval coverage: 0.64
```

Read that first line again. The GRU needed 23.7 seconds of training before it could forecast anything — and pays again for every new dataset. TimesFM produced all 120 forecasts, for series it had never seen, in six and a half seconds.
...
The switch is more than aesthetic; it changes the economics. A transformer must re-attend over the whole context for every new observation, at cost quadratic in context length; a recurrent model *carries its history* in a fixed-size hidden state and absorbs each new patch at constant cost. TiRex's natural habitat is therefore streaming and edge deployment — its makers benchmark it on industrial PLC hardware, the rugged controllers that run factory equipment on a few gigabytes of RAM — and the whole model is only **35M parameters**, a sixth of TimesFM, yet competitive with the largest entries on zero-shot leaderboards like GIFT-Eval.
...
Linear (trained)           no   9.46      -         -            4.7
GRU (trained)              no   9.37      -         -           23.7
TimesFM 2.5               yes   8.30    1.98       0.79          6.5
TiRex                     yes   8.76    2.10       0.79          2.4
Chronos-2                 yes   7.74    1.86       0.80          1.8
Chronos-2 (group)         yes   8.01    1.91       0.79          1.0
```

> **[FIGURE 10 — graphs/graph14-11.png — notebook cell 79: horizontal bar chart of arena RMSE — zero-shot models in green, trained specialists in blue, the seasonal-naive baseline in red]**
...
In the bar chart, every green bar is shorter than every blue bar, with the red baseline towering over both camps — and the spread *within* the greens is smaller than the gap between greens and blues. On this kind of data, **which foundation model you pick matters less than whether you pick one.**

A clear win, then — but a modest one: the margins are fractions of an RMSE point, earned on data where the specialists had five years to study. Is there a setting where the gap becomes a chasm? There is.
...
The ranking tells the story cleanly. **The zero-shot models degrade gracefully**: TiRex and Chronos-2 lead the 60-day field — and TiRex on 60 days *beats the GRU trained on five years*. Sixty days is two months of weekly cycles: enough for a model that has seen a million weekly rhythms to lock on. **The starved specialists don't degrade — they collapse**: the 60-day linear model finishes behind seasonal naive (nineteen windows cannot estimate a 28×14 map worth having), and the 60-day GRU essentially ties the free baseline. Training on a cold series buys you complexity risk, not accuracy. And one more reading, easy to miss: **history still helps the history-lovers** — Chronos-2 with its full 512-day context remains the best number in the table. A foundation model is not a reason to throw history away; it is a way to survive not having any.
...
Sixty days in, a calibrated two-week forecast out — seesaw in place, level tracked, a sensible band around it. Rewind to episode 2 and consider what this plot would have taken: identify the seasonality, difference the trend, pick orders, fit, diagnose, iterate. Whatever reservations the next section raises, *this* — competent probabilistic forecasts on data-starved series, for free — is the genuinely new capability. (Standard caveat: one origin of one item, so this demonstrates the mechanism rather than benchmarking it; the pattern replicates at scale in the literature.)
...
We climbed from a copied week to a downloaded forecaster. Seasonal naive set the bar, and a linear model and a GRU cleared it the classical way, at the price of a training pipeline. Patching and quantization showed how a continuous series becomes tokens at all. Then three foundation models — a patched decoder, an attention-free xLSTM, and a universal in-context learner — cleared everything, zero-shot, with calibrated uncertainty thrown in for free. The cold-start test found the paradigm's true home (sixty days of history, no time to train), and the evaluation crisis reminded us to hold the trophy loosely: friendly data, possible leakage, and a free baseline that never stops being mandatory.
